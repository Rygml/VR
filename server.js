...
/**
 * PA2 — Sensor Bridge Server (iPhone / iOS)
 * -----------------------------------------
 * Receives POST requests from Sensor Logger (Choi Tsz Hei) and
 * re-broadcasts orientation data via WebSocket every 20 ms.
 *
 * Sensor Logger module: "Orientation"
 * Fields used: yaw (=azimuth), pitch, roll  (radians)
 *
 * Install: npm install ws
 * Run:     node server.js
 *
 * iPhone setup (Sensor Logger):
 *   Push URL  → http://<THIS_PC_IP>:8765/data
 *   Module    → Orientation (enable it)
 *   Frequency → maximum available
 */
